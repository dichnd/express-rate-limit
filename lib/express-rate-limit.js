...
function RateLimit(options) {
    ...
    function rateLimit(req, res, next) {
        ...
        var oriDownload= res.download;
        res.download = function () {
            oriDownload.apply(res, arguments);
            options.store.decr(key);
        };
        ...
        var oriJson = res.json;
        res.json = function () {
            oriJson.apply(res, arguments);
            options.store.decr(key);
        };

        var oriJsonp = res.jsonp;
        res.jsonp = function () {
            oriJsonp.apply(res, arguments);
            options.store.decr(key);
        };

        var oriRedirect = res.redirect;
        res.redirect = function () {
            oriRedirect.apply(res, arguments);
            options.store.decr(key);
        };

        var oriRender = res.render;
        res.render = function () {
            oriRender.apply(res, arguments);
            options.store.decr(key);
        };

        var oriSend = res.send;
        res.send = function () {
            oriSend.apply(res, arguments);
            options.store.decr(key);
        };

        var oriSendFile= res.sendFile;
        res.sendFile = function () {
            oriSendFile.apply(res, arguments);
            options.store.decr(key);
        };

        var oriSendStatus= res.sendStatus;
        res.sendStatus = function () {
            oriSendStatus.apply(res, arguments);
            options.store.decr(key);
        };
